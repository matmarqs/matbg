...
H = H_1 + H_2 + H_{\perp},
$$
where $H_\ell$ is the hamiltonian of the layer $\ell$, and $H_\perp$ describes the interlayer hybridization. Working with a tight-binding model, it is natural to write this hamiltonian in terms of Bloch waves (\textcolor{red}{mencionar $\bm{\tau}_{\ell,\alpha}$ antes})
$$
\ket{\psi_{\ell, \k, \alpha}} = \frac{1}{\sqrt{N_\ell}} \sum_{\R_\ell} e^{i \k \vdot (\R_\ell + \bm{\tau}_{\ell,\alpha})} \ket{\ell, R_\ell, \alpha},
$$
where $\ell$ labels the layer, $N_\ell$ is the number of unit cells of layer $\ell$, $\R_\ell$ are the positions of lattice sites, $\bm{\tau}_{\ell,\alpha}$ are the orbital centers in the unit cell of each layer, and $\ket{\ell,\R_\ell,\alpha}$ are localized Wannier states. In this basis, considering only nearest-neighbor intralayer hopping, the hamiltonian of each layer is given by
$$
H_\ell(\k) =
\begin{pmatrix}
0 & -t f_\ell(\k) \\
...
\end{pmatrix},
$$
with $f_\ell(\k) = \sum_{i=1}^{3} e^{i\k \vdot \bm{\delta}_\ell}$.

Planning to describe the low energy states, we write $\k = \pm \K_\ell + \q$, where $\pm \K_\ell$ are the Dirac points of each layer.
