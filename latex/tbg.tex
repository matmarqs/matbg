...
\end{pmatrix},
$$
is the rotation matrix and we use the reference frame where $\theta_1 = -\theta/2$ and $\theta_2 = +\theta/2$.
...
\textcolor{red}{\vb{b}_2^{\text{m}} = \sqrt{3} \abs{\Delta \K} \qty(\frac{1}{2}, -\frac{\sqrt{3}}{2}),}
$$
where $\abs{\Delta \K} = 2 \abs{\K} \sin(\theta/2)$ and $\abs{\K} = 4\pi/(3\sqrt{3} d)$.

The area of the moiré lattice unit cell is
$$
A_{\text{m.u.c.}} = \frac{(2\pi)^2}{\abs{\b_1^{\text{m}} \cross \b_2^{\text{m}}}} = \frac{3\sqrt{3} d^2}{8 \sin[2](\theta/2)}.
$$
...
H = H_1 + H_2 + H_{\perp},
$$
where $H_\ell$ is the hamiltonian of the layer $\ell$, and $H_\perp = V_{12} + V_{12}^\d$ describes the interlayer hybridization. Working with a tight-binding model, it is natural to write this hamiltonian in terms of Bloch waves
$$
